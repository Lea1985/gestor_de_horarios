...
export default function ReporteProfesorPage() {
  ...
  const clavesFiltros = () => JSON.stringify({ agenteId })
  ...
  const handleVer = () => {
    ...
    setFiltrosVistos(clavesFiltros())
  }
          ...
          <div style={{ padding: "var(--space-4)", display: "flex", flexDirection: "column", gap: "var(--space-6)" }}>
            {cargando ? (
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "var(--space-2)", padding: "10px 14px", borderRadius: "var(--radius-md)", background: "var(--color-surface-raised)", border: "1px solid var(--color-border-strong)", fontSize: "var(--text-xs)", color: "var(--color-text-secondary)" }}>
                El profesor seleccionado cambió desde que se generó esta vista. Volvé a "Ver en pantalla" para actualizarla — si descargás el PDF ahora, va a ser del profesor nuevo, no del que ves acá abajo.
              </div>
            ) : errorVista ? (
