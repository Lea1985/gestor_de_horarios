...
const FORM_VACIO: FormData = { codigoUnidad: "", nombre: "", tipo: "" }
...
function BadgeTipo({ tipo }: { tipo: string | null }) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", padding: "3px 8px", borderRadius: "20px", fontSize: "var(--text-xs)", fontWeight: "var(--font-medium)", background: colors.bg, color: colors.color }}>
      {tipo}
    </span>
  )
}
...
export default function UnidadesPage() {
  ...
  function abrirEditar(u: Unidad) {
    setForm({ codigoUnidad: String(u.codigoUnidad), nombre: u.nombre, tipo: u.tipo ?? "" })
    setFormErrors({}); setEditando(u.id); setMostrarForm(true); setError(null)
  }
  ...
  function validar(): boolean {
    ...
    if (!editando && !form.codigoUnidad.trim()) errors.codigoUnidad = "El código es requerido"
    else if (!editando && isNaN(Number(form.codigoUnidad))) errors.codigoUnidad = "El código debe ser un número"
  ...
  async function guardar() {
    ...
    try {
      ...
      const body   = { nombre: form.nombre, tipo: form.tipo || undefined, ...(!editando && { codigoUnidad: Number(form.codigoUnidad) }) }
  ...
  async function eliminar(id: number) {
    try {
      const res = await fetch(`/api/unidades/${id}`, { method: "DELETE", headers: authHeaders })
      if (!res.ok) {
        const data = await res.json()
        ...
        {mostrarForm && (
          <div style={{ background: "var(--color-surface)", border: "1px solid var(--color-border)", borderRadius: "var(--radius-xl)", padding: "var(--space-6)", maxWidth: 480 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
              {!editando && (
                <div>
                  <label htmlFor="field-codigo" style={s.label}>Código <span style={{ color: "var(--color-error)" }}>*</span></label>
                  <input id="field-codigo" type="number" value={form.codigoUnidad}
                    onChange={e => { setForm(prev => ({ ...prev, codigoUnidad: e.target.value })); if (formErrors.codigoUnidad) setFormErrors(prev => ({ ...prev, codigoUnidad: undefined })) }}
                    style={{ ...s.input, ...(formErrors.codigoUnidad ? s.inputError : {}) }}
                    onFocus={e => { e.target.style.borderColor = "var(--color-accent)"; e.target.style.boxShadow = "0 0 0 3px rgba(30,155,184,0.12)" }}
                    onBlur={e => { e.target.style.borderColor = formErrors.codigoUnidad ? "var(--color-error)" : "var(--color-border)"; e.target.style.boxShadow = "none" }}
                    placeholder="Ej: 101"
                  />
                  {formErrors.codigoUnidad && <span style={{ fontSize: "var(--text-xs)", color: "var(--color-error)", marginTop: "var(--space-1)", display: "block" }}>{formErrors.codigoUnidad}</span>}
                </div>
              )}
              <div>
                <label htmlFor="field-nombre" style={s.label}>Nombre <span style={{ color: "var(--color-error)" }}>*</span></label>
                <input id="field-nombre" value={form.nombre}
                  onChange={e => { setForm(prev => ({ ...prev, nombre: e.target.value })); if (formErrors.nombre) setFormErrors(prev => ({ ...prev, nombre: undefined })) }}
                  style={{ ...s.input, ...(formErrors.nombre ? s.inputError : {}) }}
                  onFocus={e => { e.target.style.borderColor = "var(--color-accent)"; e.target.style.boxShadow = "0 0 0 3px rgba(30,155,184,0.12)" }}
              ...
              <div>
                ...
                <select id="field-tipo" value={form.tipo} onChange={e => setForm(prev => ({ ...prev, tipo: e.target.value }))}
