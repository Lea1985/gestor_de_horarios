...
import {
  crearReemplazo,
  DatosReemplazoInvalidosError,
  SuplenteRequeridoError,
  ClaseNoEncontradaError,
  AsignacionTitularNoEncontradaError,
...
export async function POST(req: Request) {
  return withContext(req, async ({ tenantId }) => {
    ...
    try {
      ...
      if (
        error instanceof DatosReemplazoInvalidosError ||
        error instanceof SuplenteRequeridoError
      ) {
        return Response.json({ error: (error as Error).message }, { status: 400 })
      }
      if (
        error instanceof ClaseNoEncontradaError ||
        error instanceof AsignacionTitularNoEncontradaError ||
        error instanceof AgenteSuplenteNoEncontradoError 
      ) {
