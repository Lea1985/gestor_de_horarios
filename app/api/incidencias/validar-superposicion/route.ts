...
export async function POST(req: Request) {
  return withContext(req, async ({ tenantId }) => {
    ...
    try {
      ...
      console.error("Error en validar-superposicion:", error)
      return Response.json({ error: "Error procesando incidencia" }, { status: 500 })
    }
  })
}
...
export async function GET(req: Request) {
  return withContext(req, async ({ tenantId }) => {
    ...
    const { listarIncidencias } = await import("@/lib/usecases/incidencias/listarIncidencias")
