...
export async function PATCH(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  return withContext(req, async ({ tenantId }) => {
    ...
    try {
      ...
      if (error instanceof EditarHijosError || error instanceof EditarReemplazosError) {
...
export async function DELETE(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  return withContext(req, async ({ tenantId }) => {
    try {
      ...
      if (error instanceof TieneHijosError) {
        return Response.json({ error: (error as Error).message }, { status: 409 })
      }
      if (error instanceof TieneReemplazosError) {
        return Response.json({ error: (error as Error).message }, { status: 409 })
      }
      console.error("Error eliminando incidencia:", error)
      return Response.json({ error: "Error eliminando incidencia" }, { status: 500 })
    }
  })
}