...
export async function GET(
  ...
  return withContext(req, async ({ tenantId }) => {
    try {
      ...
      return Response.json({ error: "Error obteniendo período" }, { status: 500 })
    }
  })
}
...
export async function PATCH(
  ...
  return withContext(req, async ({ tenantId }) => {
    ...
    try {
      ...
      return Response.json({ error: "Error actualizando período" }, { status: 500 })
    }
  })
}
...
export async function DELETE(
  ...
  return withContext(req, async ({ tenantId }) => {
    try {
      ...
      if (error instanceof NoSePuedeEliminarPeriodoActivoError) {
        return Response.json({ error: error.message }, { status: 409 })
      }
      console.error("Error eliminando período:", error)
      return Response.json({ error: "Error eliminando período" }, { status: 500 })
    }
  })
}
