...
export class DatosModuloInvalidosError extends Error {
  constructor() {
    super("dia_semana, hora_desde y hora_hasta son obligatorios")
  }
}

export class HorasInvalidasError extends Error {
  constructor() {
    super("hora_desde debe ser menor que hora_hasta")
  }
}

export class SolapamientoError extends Error {
  constructor() {
    super("Horario solapado con otro módulo existente")
  }
}
...
export class DiaInvalidoError extends Error {
  constructor() {
    super(`dia_semana inválido. Válidos: ${DIAS_VALIDOS.join(", ")}`)
  }
}
...
export async function crearModulo(
  ...
  if (!DIAS_VALIDOS.includes(dia_semana as Dias)) {
    throw new DiaInvalidoError()
  }

  if (hora_desde >= hora_hasta) {
    throw new HorasInvalidasError()
  }

  const existente = await moduloHorarioRepository.obtenerPorDatosDuplicado(
    tenantId,
    dia_semana as Dias,
    hora_desde,
    hora_hasta
  ...
  if (existente) {
    if (existente.deletedAt) {
      return moduloHorarioRepository.actualizar(
        existente.id,
        tenantId,
        {
          deletedAt: null,
  ...
  if (
    await moduloHorarioRepository.haySolapamiento(
      tenantId,
      dia_semana as Dias,
      hora_desde,
      hora_hasta
  ...
  return moduloHorarioRepository.crear({
    tenantId,
